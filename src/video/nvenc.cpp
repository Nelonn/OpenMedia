...
public:
  ...
  auto configure(const EncoderOptions& options) -> OMError override {
    ...
    init_params.frameRateNum = 30; // Default
    ...
    // Apply rate control from options
  ...
  auto encode(const Frame& frame) -> Result<std::vector<Packet>, OMError> override {
    ...
    if (nvenc_.nvEncLockInputBuffer(encoder_, &lock_params) == NV_ENC_SUCCESS) {
      ...
      for (int i = 0; i < picture.planes.getPlaneCount(); ++i) {
        ...
        for (uint32_t y = 0; y < dims.second; ++y) {
          std::memcpy(dst + y * dst_stride, src_ptr + y * src_stride, dims.first * getBytesPerPixel(format_, i));
        }
    ...
    pic_params.inputBuffer = surf.input_ptr;
    pic_params.bufferFmt = (format_ == OM_FORMAT_P010) ? NV_ENC_BUFFER_FORMAT_YUV420_10BIT : NV_ENC_BUFFER_FORMAT_NV12;
    ...
    if (nvenc_.nvEncEncodePicture(encoder_, &pic_params) != NV_ENC_SUCCESS) {
...
private:
  auto encodeDX11(std::shared_ptr<DX11HardwarePicture> pic, int64_t pts) -> Result<std::vector<Packet>, OMError> {
    auto* cu = NVLoader::getInstance().cuda();
    if (!cu->cuGraphicsD3D11RegisterResource) return Err(OM_CODEC_ENCODE_FAILED);

    CUgraphicsResource cu_resource;
    if (cu->cuGraphicsD3D11RegisterResource(&cu_resource, pic->pic->texture, CU_GRAPHICS_REGISTER_FLAGS_NONE) != CUDA_SUCCESS) {
      return Err(OM_CODEC_ENCODE_FAILED);
    }
    ...
    reg.height = height_;
    reg.pitch = width_;
    reg.bufferFormat = (format_ == OM_FORMAT_P010) ? NV_ENC_BUFFER_FORMAT_YUV420_10BIT : NV_ENC_BUFFER_FORMAT_NV12;
    ...
    NV_ENC_PIC_PARAMS pic_params = {};
    pic_params.version = NV_ENC_PIC_PARAMS_VER;
    pic_params.inputBuffer = map.mappedResource;
    pic_params.bufferFmt = reg.bufferFormat;
    pic_params.inputWidth = width_;
    pic_params.inputHeight = height_;
    pic_params.outputBitstream = out.output_ptr;
    pic_params.inputTimeStamp = pts;

    nvenc_.nvEncEncodePicture(encoder_, &pic_params);
  ...
  auto encodeCuda(std::shared_ptr<CudaHardwarePicture> pic, int64_t pts) -> Result<std::vector<Packet>, OMError> {
    NV_ENC_REGISTER_RESOURCE reg = {};
    reg.version = NV_ENC_REGISTER_RESOURCE_VER;
    reg.resourceType = NV_ENC_INPUT_RESOURCE_TYPE_CUDADEVICEPTR;
    ...
    reg.pitch = pic->pitch;
    reg.bufferFormat = (pic->format == OM_FORMAT_P010) ? NV_ENC_BUFFER_FORMAT_YUV420_10BIT : NV_ENC_BUFFER_FORMAT_NV12;
    reg.bufferUsage = NV_ENC_INPUT_IMAGE;
